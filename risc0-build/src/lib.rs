...
impl Risc0Method {
    pub fn make_method_id(&self, code_limit: u32) -> Vec<u8> {
...
pub fn get_package<P>(manifest_dir: P) -> Package
where
    P: AsRef<Path>,
{
...
pub fn guest_methods<P>(pkg: &Package, out_dir: P) -> Vec<Risc0Method>
where
    P: AsRef<Path>,
{
...
pub struct GuestBuildEnv {
    ...
    pub rust_lib_src: PathBuf,
}

pub fn setup_guest_build_env<P>(out_dir: P) -> GuestBuildEnv
where
    P: AsRef<Path>,
{
    ...
    P: AsRef<Path>,
{
    ...
    if let Some(tty) = &mut tty {
        ...
        .unwrap();
        write!(tty, "Building guest package: cargo {}", args.join(" ")).unwrap();
    }
